...
$f_cleanup_invalid    = gpc_get_bool( 'cleanup_invalid', false );
	...
	switch( $p_type ) {
		case 'project':
			...
			$t_query = "SELECT f.*
				FROM $t_file_table f
				WHERE content <> ''
				  AND content <> '\'\'' -- ignoring invalid BLOBs see #20340
...
function cleanup_invalid_attachments( $p_type ) {
	$t_file_table = db_get_table( "mantis_${p_type}_file_table" );
	...
	$t_count = reset( $t_result );

	if( $t_count > 0 ) {
		echo "<p class=\"bold\">Cleaning up invalid attachments</p>\n";

		$t_update_query = "UPDATE $t_file_table SET content = '' WHERE content = '\'\''";
		$t_update_result = db_query_bound( $t_update_query );

		if( $t_update_result ) {
			echo "$t_count records processed.\n";
		}
		else {
...
if( $f_cleanup_invalid ) {
	$t_cleanup = cleanup_invalid_attachments( $f_file_type );
} else {
	$t_cleanup = false;
}
