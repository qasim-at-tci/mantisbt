...
/**
 * This MantisBT custom rule is based on PHP_CodeSniffer 2.5
...
/**
 * Mantis_Sniffs_Functions_FunctionCallArgumentSpacingSniff
 *
...
class Mantis_Sniffs_Functions_FunctionCallArgumentSpacingSniff implements PHP_CodeSniffer_Sniff
{
	...
	public function process(PHP_CodeSniffer_File $phpcsFile, $stackPtr)
	{
		...
		// No whitespace allowed between function name and opening parenthesis
		...
		// There must be 1 space after opening parenthesis
		if(    $tokens[$openBracket+1]['code'] !== T_WHITESPACE
			&& $tokens[$openBracket+1]['code'] !== T_CLOSE_PARENTHESIS
		) {
			$error = 'No whitespace found after ( in function call to %s';
			$data  = array($tokens[$functionName]['content']);
			$phpcsFile->addError($error, $stackPtr, 'TooMuchSpace', $data);
		} else {
		...
		// There must be 1 space before closing parenthesis
		$closeBracket = $tokens[$openBracket]['parenthesis_closer'];
		if(    $tokens[$closeBracket-1]['code'] !== T_WHITESPACE
			&& $tokens[$closeBracket-1]['code'] !== T_OPEN_PARENTHESIS
		) {
			$error = 'No whitespace found before ) in function call to %s';
			$data  = array($tokens[$functionName]['content']);
			$phpcsFile->addError($error, $stackPtr, 'TooMuchSpace', $data);
		} else {
